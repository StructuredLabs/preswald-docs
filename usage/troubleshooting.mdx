...
This guide provides solutions to common issues you may encounter while using Preswald. Follow the steps below to troubleshoot and resolve problems effectively.
...
If you encounter issues related to Docker, ensure that Docker is installed and running on your system.
...
If deploying to Google Cloud Run fails, ensure that the Google Cloud CLI (gcloud) is installed and configured correctly.
...
- **Project Configuration**: Ensure you have selected the correct project using `gcloud config set project <project-id>`.
...
3. **Install Dependencies**:
   Run the following command to install dependencies listed in your `requirements.txt`:

   ```bash
   pip install -r requirements.txt
   ```

   Alternatively, if you have a `conda` environment file (e.g., `environment.yml`), use:
...
2. **Verify File Paths**:  
   Ensure all file paths referenced in your script (e.g., `data.csv`) are correct and accessible.
...
- [Contact us](mailto:amrutha@structuredlabs.com)
